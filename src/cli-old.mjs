  ...
  subCommands: {
    ...
    generate: defineCommand({
      ...
      args: {
        template: {
          type: 'positional',
          description: 'Template to generate (test, scenario, cli, config, project)',
          required: true,
        },
        name: {
          type: 'positional',
      ...
      run: async (ctx) => {
        ...
        try {
          ...
          let templateFile, outputFile, templateData

          switch (template.toLowerCase()) {
            case 'test':
              templateFile = 'test/basic.test.njk'
              outputFile = join(outputDir, `${name}.test.${format}`)
              templateData = {
                name,
            ...
            case 'scenario':
              templateFile = 'scenario/custom.scenario.njk'
              outputFile = join(outputDir, `${name}.scenario.${format}`)
              templateData = {
                name,
                title: `${name
                  .replace(/-/g, ' ')
                  .replace(/\b\w/g, (l) => l.toUpperCase())} Scenario`,
            ...
            case 'cli':
              templateFile = 'cli/basic.cli.njk'
              outputFile = join(outputDir, `${name}.cli.${format}`)
              templateData = {
                name: name.replace(/-/g, ''),
...
if (json) {
  ...
  console.log(result.message)
            ...
            case 'config':
              templateFile = 'config/package.json.njk'
              outputFile = join(outputDir, 'package.json')
              templateData = {
                name,
                version,
            ...
            case 'project':
              ...
              templateFile = 'config/package.json.njk'
              outputFile = join(projectDir, 'package.json')
              templateData = {
                name,
                version,
              ...
              // Generate CLI file
              const cliTemplateFile = 'cli/basic.cli.njk'
              const cliOutputFile = join(projectDir, `src/${name}.${format}`)
              const cliTemplateData = {
                name: name.replace(/-/g, ''),
                cliName: name,
                version,
                description: description || `CLI project: ${name}`,
...
if (json) {
  ...
  console.log(result.message)
              ...
              // Generate test file
              const testTemplateFile = 'test/basic.test.njk'
              const testOutputFile = join(projectDir, `tests/${name}.test.${format}`)
              ...
              const vitestTemplateFile = 'config/vitest.config.njk'
              const vitestOutputFile = join(projectDir, 'vitest.config.mjs')
              const vitestTemplateData = {
                environment: 'node',
                timeout: 10000,
                coverage: {
                  provider: 'v8',
              ...
              const packageJsonContent = nunjucks.render(templateFile, templateData)
              const cliContent = nunjucks.render(cliTemplateFile, cliTemplateData)
              const testContent = nunjucks.render(testTemplateFile, testTemplateData)
              const vitestContent = nunjucks.render(vitestTemplateFile, vitestTemplateData)

              // Write all files
              await writeFile(outputFile, packageJsonContent)
              await writeFile(cliOutputFile, cliContent)
              await writeFile(testOutputFile, testContent)
              await writeFile(vitestOutputFile, vitestContent)
            ...
            default:
              throw new Error(
                `Unknown template: ${template}. Available: test, scenario, cli, config, project`
              )
          }
          ...
          if (json) {
            ...
            console.log(`✅ Generated ${template} template: ${name}.${format}`)
          ...
          if (json) {
            ...
            console.error(`❌ Failed to generate ${template} template: ${name}.${format}`)
