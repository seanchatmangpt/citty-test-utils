...
import { EnterprisePerformanceMonitor } from '../performance/enterprise-performance-monitor.js'
...
export class EnterprisePerformanceTestRunner extends EnterpriseTestRunner {
  ...
  wrapWithLoadTesting(testFn) {
    return async () => {
      const concurrency = 5 // Reduced concurrency for testing
  ...
  generatePerformanceSummary() {
    ...
    if (metrics.length > 0) {
      ...
      summary.memoryUsage.average =
        memoryUsages.reduce((sum, m) => sum + m, 0) / memoryUsages.length

      // Check threshold violations
      summary.thresholdViolations = metrics.filter(
        (m) => m.duration > this.performanceThresholds.commandExecution
      )
    }
  ...
  generatePerformanceRecommendations() {
    ...
    const slowTests = metrics.filter(
      (m) => m.duration > this.performanceThresholds.commandExecution * 2
    )
