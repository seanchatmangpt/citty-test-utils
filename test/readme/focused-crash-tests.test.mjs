...
describe('Focused Crash Tests - Let It Crash Philosophy', () => {
  ...
  describe('Assertion Failure Crash Tests', () => {
    it('should crash on output assertion failures', async () => {
      if (!cleanroomSetup) {
        console.log('⏭️ Skipping test - cleanroom not available')
        return
      }

      // This should crash if output assertion fails
      const result = await runCitty(['--version'], { env: { TEST_CLI: 'false' } })
      
      // Let it crash - this should fail because we expect wrong output
      await expect(() => {
        result.expectOutput('wrong-version')
      }).toThrow()
      
      console.log('✅ Output assertion failure properly surfaced')
    })

    it('should crash on stderr assertion failures', async () => {
      if (!cleanroomSetup) {
        console.log('⏭️ Skipping test - cleanroom not available')
        return
      }

      // This should crash if stderr assertion fails
      const result = await runCitty(['nonexistent-command'], { env: { TEST_CLI: 'false' } })
      
      // Let it crash - this should fail because we expect wrong stderr
      await expect(() => {
        result.expectStderr('wrong-error-message')
      }).toThrow()
      
      console.log('✅ Stderr assertion failure properly surfaced')
    })
  })
  ...
  describe('Crash Test Summary', () => {
    it('should demonstrate comprehensive crash testing', async () => {
      ...
      console.log('   ✅ Assertion failures properly surfaced')
